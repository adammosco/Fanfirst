...
function Toast({ toasts }) {
  return (
    <div style={{ position:"fixed", top:16, left:"50%", transform:"translateX(-50%)", zIndex:999, display:"flex", flexDirection:"column", gap:8, alignItems:"center", pointerEvents:"none", width:"90%", maxWidth:400 }}>
      {toasts.map(t => (
        <div key={t.id} style={{ background:t.type==="error" ? T.red : t.type==="success" ? T.green : T.card, color:t.type==="error"||t.type==="success" ? "#fff" : T.text, border:`1px solid ${T.border}`, borderRadius:12, padding:"10px 18px", fontSize:13, fontWeight:600, boxShadow:"0 4px 20px #00000066", animation:"slideDown 0.3s ease", textAlign:"center" }}>
          {t.msg}
        </div>
      ))}
  ...
  return (
    <div style={{ marginBottom:12 }}>
      ...
      <div style={{ background:T.border, borderRadius:99, height:5 }}>
        <div style={{ width:`${clamp((value/max)*100,0,100)}%`, background:color, height:"100%", borderRadius:99, transition:"width 1s ease", boxShadow:`0 0 8px ${color}55` }}/>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#00000099", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:300, backdropFilter:"blur(8px)" }}
      ...
      <div style={{ background:T.card, width:"100%", maxWidth:480, borderRadius:"22px 22px 0 0", border:`1px solid ${T.border}`, maxHeight:"90vh", overflowY:"auto" }}>
        {!noPad ? <div style={{ padding:"24px 20px 40px" }}>{children}</div> : children}
      </div>
    </div>
  );
}
      ...
      {!done ? (
        ...
        <div style={{ textAlign:"center" }}>
          ...
          <p style={{ color:T.muted, fontSize:13, lineHeight:1.6, margin:"0 0 24px" }}>
            {finalScore>=70 ? "You're a top-tier fan. Early access opens 2 weeks before the public sale." :
             finalScore>=40 ? "Loyal fan status earned. You get access 1 week before the public sale." :
             "You're in! Build your score by linking Spotify and attending more shows."}
          </p>
    ...
    <Modal open={open} onClose={onClose}>
      {step===0 && (
        <>
          <div style={{ textAlign:"center", marginBottom:20 }}>
            ...
            <p style={{ color:T.muted, fontSize:13, lineHeight:1.6, margin:0 }}>Required to purchase tickets. Your details are encrypted and never shared with third parties.</p>
          </div>
      ...
      {step===1 && (
        <>
          ...
          <button onClick={submit} disabled={loading} style={{ width:"100%", background:loading?T.border:T.gold, border:"none", borderRadius:14, padding:"14px 0", fontWeight:800, fontSize:15, color:loading?T.muted:T.bg, cursor:loading?"not-allowed":"pointer", transition:"all 0.2s" }}>
      ...
      <div style={{ position:"relative" }}>
        ...
        <div style={{ padding:"20px 20px 36px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:18 }}>
            {[["📍",concert.venue],["📅",`${concert.date} · ${concert.time}`],["🎟",`${concert.remaining} tickets remaining`]].map(([icon,text])=>(
              <div key={text} style={{ display:"flex", alignItems:"center", gap:10, fontSize:13, color:T.muted }}>
          ...
          <div style={{ marginBottom:18 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
              ...
              <span style={{ fontSize:11, color:almostGone?T.red:T.muted, fontWeight:almostGone?700:400 }}>{almostGone?"⚡ Almost gone!":""}</span>
            </div>
            <div style={{ background:T.border, borderRadius:99, height:5 }}>
...
function ConcertCard({ concert, userTier, onBuy, onExpand, waitlisted, onWaitlist }) {
  ...
  return (
    <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:20, overflow:"hidden", marginBottom:14, transition:"transform 0.2s, border-color 0.2s", cursor:"pointer" }}
    ...
    <Modal open={open} onClose={onClose} noPad>
      <div style={{ position:"relative", height:140 }}>
        ...
        <div style={{ position:"absolute", bottom:12, left:16 }}>
          ...
          <div style={{ fontSize:12, color:"#ffffff88" }}>{concert.venue} · {concert.date}</div>
        </div>
      </div>
      <div style={{ padding:"20px 20px 36px" }}>
        {!purchased ? (
          <>
            {!verified && (
              <div style={{ background:T.red+"18", border:`1px solid ${T.red}44`, borderRadius:12, padding:"10px 14px", marginBottom:14, fontSize:12, color:T.red }}>
                ⚠️ You need to verify your identity before purchasing. Tap Confirm to start.
              </div>
            )}
          ...
          <div style={{ textAlign:"center", padding:"12px 0" }}>
            ...
            <p style={{ color:T.muted, fontSize:14, lineHeight:1.6 }}>
              {qty} × {concert.artist} ticket{qty>1?"s":""} confirmed.<br/>Your named ticket arrives 48hrs before the show.
            </p>
...
function SettingsDrawer({ open, onClose, user, localPhoto, onPhotoChange, onSignOut, spotifyLinked, setSpotifyLinked, appleLinked, setAppleLinked, notifications, setNotifications, emailAlerts, setEmailAlerts, earlyAccess, setEarlyAccess, score, userTier, showToast }) {
  ...
  function SLabel({ t }) { return <div style={{ fontSize:10, color:T.muted, letterSpacing:1.8, fontWeight:700, textTransform:"uppercase", padding:"16px 20px 6px" }}>{t}</div>; }
    ...
    <>
      ...
      <div style={{ position:"fixed", top:0, right:0, bottom:0, width:"88%", maxWidth:360, background:T.surface, zIndex:201, transform:open?"translateX(0)":"translateX(100%)", transition:"transform 0.3s cubic-bezier(0.4,0,0.2,1)", overflowY:"auto", borderLeft:`1px solid ${T.border}` }}>
        ...
        <div style={{ background:`linear-gradient(160deg,${T.card} 0%,#1A1020 100%)`, padding:"52px 20px 24px", position:"relative" }}>
          ...
          <div style={{ position:"relative", display:"inline-block", marginBottom:14 }}>
            ...
            <input ref={fileRef} type="file" accept="image/*" style={{ display:"none" }}
              onChange={e=>{ const f=e.target.files?.[0]; if(f){ onPhotoChange(URL.createObjectURL(f)); showToast("Profile photo updated ✓","success"); } }}/>
          </div>
        ...
        <div style={{ paddingBottom:40 }}>
          ...
          <Row icon="🟢" label="Spotify" sublabel={spotifyLinked?"Connected · streaming score active":"Not connected — link to boost score"} right={<Toggle value={spotifyLinked} onChange={v=>{ setSpotifyLinked(v); showToast(v?"Spotify connected ✓":"Spotify disconnected","success"); }}/>}/>
          ...
          <Row icon="🔔" label="Push Notifications" right={<Toggle value={notifications} onChange={v=>{ setNotifications(v); showToast(v?"Push notifications on":"Push notifications off"); }}/>}/>
          ...
          <SLabel t="Account"/>
          <Row icon="🪪" label="Verify Identity" sublabel="Required for ticket purchases" tag="REQUIRED" onClick={()=>{ onClose(); showToast("Opening identity verification..."); }}/>
          ...
          <Row icon="💬" label="Contact Support" sublabel="Usually replies within 2 hours" onClick={()=>{ showToast("Opening support chat..."); }}/>
...
function MainApp() {
  ...
  // Init display name from Clerk
  useEffect(()=>{
    if (user?.firstName) setDisplayName(`${user.firstName} ${user.lastName??""}`.trim());
  }, [user]);

  const filteredConcerts = CONCERTS_DATA.filter(c => {
    const matchSearch = c.artist.toLowerCase().includes(search.toLowerCase()) || c.venue.toLowerCase().includes(search.toLowerCase());
    const matchGenre = genreFilter==="All" || c.genre===genreFilter;
      ...
      <div style={{ padding:"18px 14px" }}>
        ...
        {tab==="discover" && (
          <>
            {/* Score banner */}
            {!quizDone ? (
              <div style={{ background:`linear-gradient(135deg,${T.card} 0%,#1A0F28 100%)`, border:`1px solid ${T.borderHi}`, borderRadius:18, padding:"18px 18px", marginBottom:18 }}>
                <div style={{ fontSize:15, fontWeight:800, color:T.text, marginBottom:6 }}>What's your Fan Score?</div>
                <p style={{ fontSize:13, color:T.muted, margin:"0 0 14px", lineHeight:1.6 }}>Take a 2-minute quiz to unlock your tier and get early access to tickets.</p>
            ...
            <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:4, marginBottom:18 }}>
              {GENRES.map(g=>(
                <button key={g} onClick={()=>setGenreFilter(g)} style={{ background:genreFilter===g?T.gold:T.card, color:genreFilter===g?T.bg:T.muted, border:`1px solid ${genreFilter===g?T.gold:T.border}`, borderRadius:99, padding:"6px 14px", fontSize:12, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap", transition:"all 0.15s", flexShrink:0 }}>
                  {g}
                </button>
              ))}
            </div>
            ...
            {filteredConcerts.length===0 ? (
              <div style={{ textAlign:"center", padding:"60px 20px", color:T.muted }}>
                ...
                <button onClick={()=>{ setSearch(""); setGenreFilter("All"); }} style={{ marginTop:8, background:T.gold, border:"none", borderRadius:10, padding:"10px 20px", fontWeight:700, fontSize:13, color:T.bg, cursor:"pointer" }}>Clear filters</button>
              </div>
          ...
          <>
              ...
              {[{ tier:1, req:"Score 70+", col:T.gold },{ tier:2, req:"Score 40+", col:T.violet },{ tier:3, req:"Any score", col:T.teal }].map(w=>{
                ...
                return (
                  <div key={w.tier} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 0", borderBottom:w.tier<3?`1px solid ${T.border}`:"none" }}>
                    <div style={{ width:36, height:36, borderRadius:99, background:active?w.col+"22":T.surface, border:`2px solid ${active?w.col:T.border}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14 }}>{active?"✓":"🔒"}</div>
                    <div style={{ flex:1 }}>
            ...
            {!verified && (
              <button onClick={()=>setVerifyOpen(true)} style={{ width:"100%", background:T.red+"18", border:`1px solid ${T.red}44`, borderRadius:14, padding:"13px 0", color:T.red, fontSize:14, cursor:"pointer", fontWeight:700, marginBottom:10 }}>
                ⚠️  Verify Identity to Purchase Tickets
              </button>
            )}
